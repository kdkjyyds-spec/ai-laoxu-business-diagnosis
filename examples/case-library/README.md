...
```
case-library/
├── raw-text/              # 2,182 个 PDF 的前 30 页文本（按期数分目录）
│   ├── 案例库 1]/  (253 files, 5.9 MB)
│   ├── 案例库  2 /  (643 files, 2.9 MB)
│   ├── 案例库 3]  /  (107 files, 0.7 MB)
│   └── 案例库 4]  /  (179 files, 1.0 MB)
├── extracted/             # 结构化提取结果
│   ├── all-100-cases.json        # 100 个抽样的合并提取（10 LLM + 90 程序化）
│   ├── auto-classify.json        # 2,182 个全量自动分类
